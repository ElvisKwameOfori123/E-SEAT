/* E-SEAT R1 module: 01_corrected_panel.do
...
/*******************************************************************************
* PART 2. CORRECTED PANEL CONSTRUCTION
*
...
*-------------------------------------------------------------------------------
* 2.1 Restore the raw Census missingness before applying the Ohio correction
*-------------------------------------------------------------------------------
*
...
* harvested-corn acreage and GovPayment fields first, then patch Ohio 2022 from
...
*-------------------------------------------------------------------------------
* 2.2 Apply the official Ohio 2022 correction
*-------------------------------------------------------------------------------
*
* Correction source:
...
*         All 88 county totals are reported in $1,000; the correction file stores
*         GovPayment_2022 in dollars to match the panel variable.
merge 1:1 fips_code Year using "$ohiofix", ///
...
*-------------------------------------------------------------------------------
* 2.3 Rebuild analytical variables from corrected source fields
*-------------------------------------------------------------------------------
...
capture label var PecentPlantedCorn "LEGACY zero-filled corn-share series; not used in R1 corrected analysis"
...
* Rebuild all government-payment derivatives from the corrected raw value.
...
/*******************************************************************************
* PART 2B. CORRECTION QA (Q1-Q3 + GOVERNMENT PAYMENTS)
*
* These checks VERIFY the repair. They do not decide whether the old counts can
* stand; those old counts are retired. All revised estimates are generated from
* the corrected variables above.
*******************************************************************************/

di _n as result "=========== PART 2B: CORRECTION QA ==========="

capture log close qa
log using "$qadir/qa_audit_data_correction.txt", text replace name(qa)
...
di as result "Max |corrected corn_share - harvested corn/ag land| = " ///
    max(abs(r(min)), abs(r(max)))

* Q2. Missing harvested acreage after Ohio repair.
di _n as result "--- Q2. CORRECTED MISSING CORN ACREAGE ---"
...
di as result "2022 counties with missing harvested corn acreage after repair = " r(N)

count if statefp == 39 & Year == 2022 & __acres_missing == 1
di as result "Ohio 2022 missing after repair = " r(N) " (expected: Belmont and Cuyahoga)"
assert r(N) == 2
...
preserve
    ...
    capture export excel using "$qadir/qa_missing_corn_rows_after_correction.xlsx", ///
        firstrow(variables) replace
restore

* Q3. Denominator diagnostic, corrected 2022 sample.
...
di as result "Ohio 2022 missing government-payment totals after repair = " r(N)
assert r(N) == 0
...
di as result "Part 2B complete: correction audit written to $qadir"
...
di as result "Part 2 complete: corrected analysis panel saved."
